...
public sealed class UpdateOddsEndpoint : Endpoint<UpdateOddsRequest, OddsResponse>
{
    private readonly IGrainFactory _grainFactory;

    public UpdateOddsEndpoint(IGrainFactory grainFactory)
    {
    ...
    public override void Configure()
    {
        ...
        Summary(s =>
        {
            s.Summary = "Update odds for a market";
            s.Description = "Updates odds for all or specific selections in a market";
            s.Params["marketId"] = "The unique identifier of the market";
            s.Response(200, "Odds updated successfully", example: new OddsResponse
            {
            ...
            s.Response(400, "Invalid odds values");
            s.Response(404, "Market not found");
            s.Response(409, "Market is suspended");
            s.Response(500, "Failed to update odds");
        });
    }

    public override async Task HandleAsync(UpdateOddsRequest req, CancellationToken ct)
    {
        try
        {
            ...
            var snapshot = await oddsGrain.UpdateOddsAsync(updateRequest);

            Response = MapToOddsResponse(snapshot);
        }
        catch (ArgumentException ex)
        {
            Logger.LogWarning(ex, "Invalid odds update request for market {MarketId}", Route<string>("marketId"));
        ...
        {
            Logger.LogError(ex, "Failed to update odds for market {MarketId}", Route<string>("marketId"));
